...
## STEP 0. 아키텍처 결정 (claude.ai + Claude Code 공동 검토)

시작 전에 반드시 결정. 결정 후 `CLAUDE.md` 에 해당 테이블의 방식 명시.

### Case A — Flat table 방식 (기본값)

아래 중 하나라도 해당하면 Flat table:
- [ ] 레코드 수 10K 이상
- [ ] 3개 이상 테이블 JOIN 필요
- [ ] formula / lookup / rollup 기반 필터/정렬이 핵심

### Case B — 직접 JOIN 방식

아래를 **모두** 만족할 때만 직접 JOIN:
- [ ] 레코드 수 5K 이하
- [ ] JOIN 테이블 2개 이하
- [ ] 기간 필터(최근 N개월) 로 충분

→ Works 의 기본 패턴은 Case A. Case B 는 마스터 성격의 소규모 테이블에 한정.

---

## Case A: Flat table 방식
...
#### A-2-2. `features/{page}/{page}Types.ts`
...
#### A-2-3. `features/{page}/{page}Config.ts`
...
- [ ] Link 컬럼: `fieldType: 'link'` + `linkConfig: { linkTable, fkColumn,
      searchFields, displayField, secondaryField }`
...
#### A-2-4. `features/{page}/{page}Renderers.ts`
...
- [ ] FK 정리 (UUID 컬럼 + 인덱스 + 백필)
- [ ] 필터/정렬 대상 인덱스 추가 (`created_at`, `brand_id` 등)
- [ ] RPC 시그니처 (Case A 와 동일)
  ```
  search_{table}(filters_json jsonb, sorts_json jsonb, search_term text,
                 result_offset int, result_limit int,
                 trashed_only boolean DEFAULT false)
  count_{table}(filters_json jsonb, search_term text,
                trashed_only boolean DEFAULT false)
  ```
  - JOIN 포함 (brands, products 등)
  - 기간 필터 기본값 적용 (예: `created_at > now() - interval '3 months'`)
- [ ] `field_options` 등록
- [ ] `deleted_at` 컬럼 확인

### B-2. 앱 코드 (Claude Code)

Case A 의 A-2 와 거의 동일. 차이점:
- `{page}Types.ts` 의 `Item` 은 JOIN 결과 shape
- `{page}Config.ts` 의 `col.data` 는 JOIN alias 와 일치
- Formula 컬럼 → `derived: true` 로 프론트 처리
- API route 의 RPC 이름은 `search_{table}` / `count_{table}` (flat_ prefix 없음)
- **Realtime 구독 비활성화** (직접 JOIN 은 trigger-free 경로가 없음)
...
| Link        | `fieldType: 'link'` + linkConfig (fkColumn / linkTable / displayField / searchFields) |
| Checkbox    | 기본 타입 그대로 |
| Date        | 기본 타입 그대로 |
